...
const buildMessage = (arrSign, arrDate, arrMes) => {
    const randSign = arrSign[Math.floor(Math.random()*(arrSign.length))];
    const randDate = arrDate[Math.floor(Math.random()*(arrDate.length))];
    const randMes = arrMes[Math.floor(Math.random()*(arrMes.length))]
